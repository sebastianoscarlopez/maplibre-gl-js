...
import {
    ...
    terrainDepthUniformValues,
    terrainCoordsUniformValues,
} from './program/terrain_program';
...
function drawDepth(painter: Painter, terrain: Terrain) {
    ...
    for (const tile of tiles) {
        ...
        const posMatrix = painter.transform.calculatePosMatrix(
            tile.tileID.toUnwrapped()
        );
        const uniformValues = terrainDepthUniformValues(
            posMatrix,
            terrain.getMeshFrameDelta(painter.transform.zoom)
        );
        program.draw(
            context,
            gl.TRIANGLES,
...
function drawCoords(painter: Painter, terrain: Terrain) {
    ...
    for (const tile of tiles) {
        ...
        const uniformValues = terrainCoordsUniformValues(
            posMatrix,
            255 - terrain.coordsIndex.length,
            terrain.getMeshFrameDelta(painter.transform.zoom)
        );
        program.draw(
            context,
...
function drawTerrain(painter: Painter, terrain: Terrain, tiles: Array<Tile>) {
    ...
    for (const tile of tiles) {
        ...
        //const tileCoords = lngLat2tile(tile.tileID.canonical.toLngLat(), tile.tileID.canonical.z);
        const tileCoords = window.clickCoords ? lngLat2tile(window.clickCoords, tile.tileID.canonical.z) : {};
        if (tile.tileID.canonical.x === tileCoords.x && tile.tileID.canonical.y === tileCoords.y) {
            terrainData.u_terrain_exaggeration = 2;
        }
        context.activeTexture.set(gl.TEXTURE0);
...
function lngLat2tile({lng, lat}, zoom) {
